                    ...
                    <table class="table mb-0 align-middle">
                        ...
                        <tfoot>
                            <tr class="fw-bold">
                                ...
                                <td class="text-end text-danger">{{ number_format($order->total_amount, 0, ',', '.') }}đ
                                </td>
                            </tr>
                        </tfoot>
                    </table>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body small">
                    ...
                    @if($order->note)
                    <p class="text-muted mb-0">{{ $order->note }}</p>@endif
                    <hr>
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <p>Hiện tại: <span
                            class="badge bg-{{ $labels[$order->status][1] ?? 'secondary' }}">{{ $labels[$order->status][0] ?? $order->status }}</span>
                    </p>
                    <form action="{{ route('admin.orders.updateStatus', $order->id) }}" method="POST">
                        @csrf @method('PATCH')
                        <select name="status" class="form-select mb-3">
                            @foreach($statuses as $s)
                                <option value="{{ $s }}" {{ $order->status === $s ? 'selected' : '' }}>{{ $labels[$s][0] ?? $s }}
                                </option>
                            @endforeach
                        </select>
