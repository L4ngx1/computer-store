    ...
    <div class="row justify-content-center mb-5">
        <div class="col-12 col-md-8 col-lg-6">
            ...
            <form action="{{ route('client.search') }}" method="GET" class="d-flex shadow-sm rounded-pill p-1 bg-white">
                <input type="text" name="q" class="form-control border-0 rounded-pill px-4"
                    placeholder="Nhập từ khóa tìm kiếm..." value="{{ request('q') }}" required>
                                    ...
                                    <div class="mt-auto">
                                        @if($product->sale_price)
                                            <div class="d-flex align-items-center gap-2 mb-2">
                                                <span
                                                    class="fw-bold text-danger fs-5">{{ number_format($product->sale_price, 0, ',', '.') }}đ</span>
                                                <span
                                                    class="text-muted text-decoration-line-through small">{{ number_format($product->price, 0, ',', '.') }}đ</span>
                                            </div>
                                        @else
                                            <div class="fw-bold text-primary fs-5 mb-2">{{ number_format($product->price, 0, ',', '.') }}đ
                                            </div>
                                        @endif
